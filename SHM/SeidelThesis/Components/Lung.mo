...
model Lung "Lung model with simple sinus signal"
  ...
  SHM.Shared.Components.Noise.AutoregressiveGaussianDeg2 T_r_fluct(
    ...
    r_last2=r_noise_last2, generator.samplePeriod=0.1001, generator.enableNoise=use_noise
  );
...
equation
  der(phi_r) = 1/T_r;
  when phase_end then
    T_r = T_r0 + (if use_noise then T_r_fluct.noise else 0);
    reinit(phi_r,0);
  end when;
  resp.phase = - sin(2*Modelica.Constants.pi*phi_r); // FIXME: shouldn't r_start also affect this?
  signal.activation = 0.5 * (1 - sin(2*Modelica.Constants.pi*(phi_r + r_start/T_r0)));

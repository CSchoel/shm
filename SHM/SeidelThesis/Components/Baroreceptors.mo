...
model Baroreceptors "baroreceptors measuring blood pressure"
  ...
  ) "baroreceptor saturation function";
  Real history[5] "history needed for broadening";
protected
...
equation
  for i in 1:5 loop
    history[i] = delay(signal.activation,i*0.1,i*0.1);
  end for;
  base_signal = artery.pressure - p0 +  kb * der(artery.pressure);
  sat.x = base_signal;
  sat_signal = if saturated then sat.satx else base_signal;
  signal.activation = if broadened then SHM.SeidelThesis.Functions.Broaden(sat_signal,eta=0.15,sigma=0.11) else sat_signal;

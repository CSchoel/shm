...
def filter_db(db, dname, outname):
  ...
  selname = os.path.join(dname, "selected.txt")
  with io.open(selname, "w", encoding="utf-8") as f:
    f.write("\n".join([x[0] for x in selected]))
  exname = os.path.join(dname, "excluded.txt")
  with io.open(exname, "w", encoding="utf-8") as f:
    f.write("\n".join([x[0] for x in excluded]))
  total = len(selected) + len(excluded)
  print("excluded: %d/%d samples" % (len(excluded), total))
...
def plot_measure_hists(data, dnames, alnames, plotdir, extra=None):
  ...
  for i in range(n):
    rng = (total_mean[i] - nsigma * total_std[i],
           total_mean[i] + nsigma * total_std[i])
    for j in range(len(data)):
      fname = "{0:s}_hist_{1:s}.png".format(alnames[i], dnames[j])
      ...
      pdfs.append(("genlogistic", sst.genlogistic.pdf(bins[:-1],
                   *sst.genlogistic.fit(d))))
      pdfs.append(("weibull_min", sst.weibull_min.pdf(bins[:-1],
                   *sst.weibull_min.fit(d))))
      pdfs.append(("weibull_max", sst.weibull_max.pdf(bins[:-1],
                   *sst.weibull_max.fit(d))))
...
def _compare_measures_sample(args):
  ...
  for s in range(nchunks):
    if max_chunks is not None and s > max_chunks:
      break
    ...
    def fnf(s):
      if dnames[s] is None:
        ...
        return os.path.join(dnames[s], fnt.format(s))
    ...
    sys.stdout.flush()
    # flush warnings so that they can be seen by the main thread
    sys.stderr.flush()
...
def compare_measures(dbs, names, outdir=None, nprocs=1, max_chunks=None):
  # TODO: ideal use of multiprocessing by directly submitting only
  # individual slices as work items
  ...
  for dbn, db in zip(names, dbs):
    ...
    rr_data = [(n, to_rr(db[n]),
                dnames, nbeats, max_chunks) for n in sample_names]
    if nprocs > 1:
    ...
    if not (outdir is None):
      with io.open(measurefile, "a", encoding="utf-8") as f:
        lines = [template.format(*([n] + list(x)))
                 for n, x in zip(log_names, log_data)]
        f.writelines(lines)
...
def replot_compare_hists():
  ...
  data = [np.loadtxt(os.path.join(dn, fn), delimiter=";",
                    skiprows=1, usecols=cols) for fn in names]
  alnames = ["lyap_e", "lyap_r", "sampEn", "hurst", "corrDim", "dfa"]
  ex = test_extras(test_file, alnames)
  plot_measure_hists(data, ["selected", "excluded"], alnames,
                     os.path.join(dn, "plots"), extra=ex)
